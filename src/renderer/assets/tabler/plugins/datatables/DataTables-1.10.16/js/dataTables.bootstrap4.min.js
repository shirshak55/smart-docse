...
o.page() != a.data.action && o.page(a.data.action).draw('page')
          ...
          if (c = f[l], b.isArray(c))q(d, c); else {
            g = e = ''; switch (c) { case 'ellipsis':e = '&#x2026;'; g = 'disabled'; break; case 'first':e = k.sFirst; g = c + (j > 0 ? '' : ' disabled'); break; case 'previous':e = k.sPrevious; g = c + (j > 0 ? '' : ' disabled'); break; case 'next':e = k.sNext; g = c + (j < n - 1 ? '' : ' disabled'); break; case 'last':e = k.sLast; g = c + (j < n - 1 ? '' : ' disabled'); break; default:e = c + 1, g = j === c ? 'active' : '' }e && (i = b('<li>', {'class': t.sPageButton + ' ' + g,
              id: r === 0 &&
typeof c === 'string' ? a.sTableId + '_' + c : null}).append(b('<a>', {href: '#', 'aria-controls': a.sTableId, 'aria-label': u[c], 'data-dt-idx': p, tabindex: a.iTabIndex, 'class': 'page-link'}).html(e)).appendTo(d), a.oApi._fnBindAction(i, {action: c}, m), p++)
          }
        }
